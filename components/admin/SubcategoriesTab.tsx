...
export default function SubcategoriesTab({ 
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Generar el slug automáticamente (mismo formato que ya usas)
    ...
    <div className="animate-in fade-in duration-300">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8">
        ...
        <button 
          ...
          className="bg-indigo-600 hover:bg-indigo-700 text-white px-5 py-2.5 rounded-xl font-bold shadow-md transition-all flex items-center gap-2 text-sm"
        >
              ...
              <div className="divide-y divide-zinc-100 dark:divide-zinc-800/50">
                {subcategories.filter(s => s.category_id === cat.id).length === 0 ? (
                  ...
                  subcategories.filter(s => s.category_id === cat.id).map(sub => (
                    <div key={sub.id} className="p-4 flex justify-between items-center group hover:bg-zinc-50 dark:hover:bg-zinc-800/30 transition-colors">
                      <span className="text-sm font-semibold text-zinc-700 dark:text-zinc-300">{sub.name}</span>
          ...
          <div className="bg-white dark:bg-[#111] border border-zinc-200 dark:border-zinc-800 rounded-3xl shadow-2xl max-w-sm w-full overflow-hidden flex flex-col relative animate-in zoom-in-95">
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-4">
              ...
              <button 
                type="submit" 
                disabled={loading}
                className="w-full bg-indigo-600 text-white p-3.5 rounded-xl font-bold text-sm shadow-lg shadow-indigo-500/20 hover:bg-indigo-700 disabled:opacity-50 transition-all mt-2"
              >
